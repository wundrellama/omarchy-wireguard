...
### Import picker and hostname-connect corrections

The picker handoff now defers dispatch until Quickshell updates its cached `busy` binding and reopens import review; synthetic real-Quickshell regression coverage complements the panel contracts. The daemon treats disconnected request/response sockets as transport failures rather than tunnel emergencies, while genuine controller exceptions still fail closed. Unexpected-error logs include exception types only.
